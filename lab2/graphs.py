...
#                if shortest_from_source_dict[source] == float('inf'):
#                    neighbour_cost = cost(source, neighbour)
#                else:
                neighbour_cost = shortest_from_source_dict[source] + graph.get_weight(source, neighbour)
                print(len(shortest_from_source_dict.keys()))
                if neighbour_cost < shortest_from_source_dict[neighbour]:
                    shortest_from_source_dict[neighbour] = neighbour_cost
#                    path[neighbour].update({path[source]})
                    prev_path = path[source]
                    if prev_path == []:
                        path[neighbour].append(source)#update({source:neighbour_cost})
                    else:
                        for prev in prev_path:
                            path[neighbour].append(prev)
#                        path[neighbour].append(prev_path)
                        path[neighbour].append(source)#({source:neighbour_cost})
#                    print(source, neighbour, neighbour_cost)
                else:
                    continue
            else:
                continue
...
#            exvis_shortest_from_source_dict = shortest_from_source_dict.copy()
#            exvis_shortest_from_source_dict.pop(source) #= shortest_from_source_dict.pop(source)
...
#            min_from_source = exvis_shortest_from_source_dict.keys()[exvis_shortest_from_source_dict.values().index(min_value)]
            if should_do_loop == True:
                return dijkstraloop(min_from_source)
                dijkstraloop(min_from_source)
 

    return dijkstraloop(source)
# Kanske kalla på dijkstraloop och sedan returnera path
# Lägg till att från 1: 0, 11: 2, 111:11
...
def visualize(graph, view='view', nodecolors=colormap):
...
def demo():
    ...
    minedge = [(1,2,4),(1,3,2),(1,4,7),(3,4,1),(3,5,1),(3,6,3), (3,7,2), (6,7,4)]
    for pair in minedge:
        G.set_weight(pair[0], pair[1], pair[2])
    view_shortest(G, 2, 6)
